...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, download, model_flag, resize, as_rgb, model_path, run):
    ...
    testloader = torch.utils.data.DataLoader(
        test_dataset, batch_size=512, shuffle=True, num_workers=16)
    '''
    ...
    train_dataset = DataClass(split='train', transform=data_transform, download=download, as_rgb=as_rgb)  # len=34561
    val_dataset = DataClass(split='val', transform=data_transform, download=download, as_rgb=as_rgb)  # len=6491
    test_dataset = DataClass(split='test', transform=data_transform, download=download, as_rgb=as_rgb)  # len=17778

    print(val_dataset)  # self.labels
    breakpoint()
    ...
    epoch_log = f'best epoch: {best_epoch}'

    log = '%s, %s\n' % (data_flag, model_flag) + train_log + val_log + test_log + epoch_log
    print(log)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path',
                        ...
                        type=str)  # experiments-main/weights/organamnist/resnet18_28_1.pth
